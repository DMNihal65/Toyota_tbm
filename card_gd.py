...
urlForCardRaised = "http://10.82.126.73:5051/card/find/fromDate/" + \
    str(f_y)+"-"+str(f_m)+"-"+str(f_d)+"/toDate/" + \
    str(t_y)+"-"+str(t_m)+"-"+str(t_d)
urlForCardRaised_4_yesterday = "http://10.82.126.73:5051/card/find/fromDate/" + \
...
if card["success"] == True:
    ...
    for entry in card_list:
        if entry["line"] == "Block" and entry["status"] != "complete":
            wsB["A1"] = "Summary of card raised in GD BLOCK LINE (last 30 days)"
    ...
    for entry in card_list:
        if entry["line"] == "Crank" and entry["status"] != "complete":
            wsC["A1"] = "Summary of card raised in GD CRANK LINE (last 30 days)"
    ...
    for entry in card_list:
        if entry["line"] == "Head" and entry["status"] != "complete":
            wsH["A1"] = "Summary of card raised in GD HEAD LINE (last 30 days)"
    ...
    if card_y["success"] == True:
        wsS["A11"] = "Summary of card raised in GD (Yesterday)"
...
url4mcDetails = "http://10.82.126.73:5051/head/headMachineList?d=" + \
    str(y_weekd)+"&y="+str(y_y)+"&w="+str(y_weekNo)+"&m="+str(y_m)+"&pS=P"
url4dailyStatus = "http://10.82.126.73:5051/dailyStatus?entryFor=" + \
    str(y_y)+"-"+str(y_m)+"-"+str(y_d)+"&pS=P"
url4mcDetails_OM = "http://10.82.126.73:5051/head/headMachineList?d=" + \
    str(y_weekd)+"&y="+str(y_y)+"&w="+str(y_weekNo)+"&m="+str(y_m)+"&pS=S"
url4dailyStatus_OM = "http://10.82.126.73:5051/dailyStatus?entryFor=" + \
